...
export default function PendingOrders() { 
  ...
  const loadItems = () => {
    axios.get(backend+'/api/order/getAllOrders').then( data => {
      ...
      const itemset = apidata.filter(item => item.sellerID === userID && item.status==="pending");
  ...
  return (
    ...
    <View style={ styles.container}>
      ...
      <FlatList 
        ...
        style={{width:'100%'}}
        renderItem={({item}) => <Card name={item.itemName} id={item._id} qty={item.qnty} price={item.price} key={item._id} />}
